...
gt_val_images = [None] * 6000
validation_images = {}
validation_images['300'] = [None] * len(validation_ids)
validation_images['250'] = [None] * len(validation_ids)
validation_images['100'] = [None] * len(validation_ids)
g_val_loss = np.zeros((5000, 1))
#print(result_dir)
...
for epoch in range(1):
    ...
    if epoch > 30: #after 3 epochs decrease the learning rate to move slower and avoid overshooting the minimum 
        learning_rate = 1e-5
    
    losses = []
    for ind in np.random.choice(np.random.permutation(len(train_ids)), 50):    #randomly pick images from the training set
