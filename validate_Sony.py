...
gt_images = [None] * 6000
input_images = {}
input_images['300'] = [None] * len(validation_ids)
input_images['250'] = [None] * len(validation_ids)
input_images['100'] = [None] * len(validation_ids)
g_loss = np.zeros((50, 1))
...
for epoch in range(1, 101):
    ...
    if epoch > 4:  # after 3 epochs decrease the learning rate to move slower and avoid overshooting the minimum
        learning_rate = 1e-5

    losses = []

    # randomly pick 180 images from the training set
    for ind in np.random.choice(np.random.permutation(len(validation_ids)), 3):
        ...
        if cnt == 3:
            val_loss.append(g_loss[ind])

        print("%d %d Loss=%.3f Time=%.3f" %
              (epoch, cnt, g_loss[ind], time.time() - st))
        ...
        if epoch % save_freq == 0:
            ...
            scipy.misc.toimage(temp * 255, high=255, low=0, cmin=0, cmax=255).save(
                result_dir + '%04d%05d_00_validation_%d.jpg' % (epoch, validation_id, ratio))
